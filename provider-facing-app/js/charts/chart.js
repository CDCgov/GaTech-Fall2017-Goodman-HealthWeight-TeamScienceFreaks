...
Chart.prototype = {
            ...
            if (lines[type]) {
                ...
                if (point) {
                    ...
                    if ( pct !== null && !isNaN(pct) && isFinite(pct) ) {
                        ...
                        //Sends popup to remind provider to enter a problem record
                        if(data.isLast == true && pct > .95 && window.sessionStorage.getItem('popupDone') != 'true' ){
                            setTimeout(function() {
                                alert("The recorded BMI for this patient is >95th percentile." +
                                    " Consider addition of obesity to the child’s Problem List."); }, 5000);
                            //alert("The recorded BMI for this patient is >95th percentile. Consider addition of obesity to the child’s Problem List.");
                            window.sessionStorage.setItem('popupDone','true');
                        }
                    }
    ...
    drawDataLineLabel : function(x, y, percentile) {
        var elem, txt = GC.App.getPCTZ() == "z" ? 
            ...
            String((Number(percentile) *100).toPrecision(3));
    ...
    drawFillChartRegion : function(data)
    {
        ...
        var l = data.length;
        if (l == 18) {
            var data1 = data.slice(0, l - 12);
            var data2 = data.slice(l - 13, l - 10);
            var data3 = data.slice(l - 10, l);
            //this.css(this.drawRegion(data), ".fill-region");
            //Shade the region between 5% to 85% green
            this.drawRegion(data1).attr(this.settings.fillRegion);
            //Shade the region between 85% and 95% yellow
            this.drawRegion(data2).attr(this.settings.fillRegion85);
            //Shade the region above 95% red
            this.drawRegion(data3).attr(this.settings.fillRegion95);
        }
        else {
